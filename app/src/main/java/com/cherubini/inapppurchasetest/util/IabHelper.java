...
package com.cherubini.inapppurchasetest.util;
...
import com.cherubini.inapppurchasetest.InAppBuyIntention;
import com.cherubini.inapppurchasetest.InAppPurchaseData;
...
public class IabHelper {
    ...
    public void launchPurchaseFlow(Activity act, String sku, int requestCode, OnIabPurchaseFinishedListener listener, InAppBuyIntention intention)
            throws IabAsyncInProgressException, JSONException {
        launchPurchaseFlow(act, sku, requestCode, listener, intention, "");
    }

    public void launchPurchaseFlow(Activity act, String sku, int requestCode,
                                   OnIabPurchaseFinishedListener listener, InAppBuyIntention intention, String extraData)
            throws IabAsyncInProgressException, JSONException {
        launchPurchaseFlow(act, sku, ITEM_TYPE_INAPP, null, requestCode, listener, intention, extraData);
    }

    public void launchSubscriptionPurchaseFlow(Activity act, String sku, int requestCode,
                                               InAppBuyIntention intention, OnIabPurchaseFinishedListener listener) throws IabAsyncInProgressException, JSONException {
        launchSubscriptionPurchaseFlow(act, sku, requestCode, listener, intention, "");
    }

    public void launchSubscriptionPurchaseFlow(Activity act, String sku, int requestCode,
                                               OnIabPurchaseFinishedListener listener, InAppBuyIntention intention, String extraData)
            throws IabAsyncInProgressException, JSONException {
        launchPurchaseFlow(act, sku, ITEM_TYPE_SUBS, null, requestCode, listener, intention , extraData);
    }
...
*                    {@link Activity#startActivityForResult}).
    ...
    public void launchPurchaseFlow(Activity act, String sku, String itemType, List<String> oldSkus,
                                   int requestCode, OnIabPurchaseFinishedListener listener, InAppBuyIntention intention, String extraData)
            throws IabAsyncInProgressException, JSONException {
        checkNotDisposed();
        ...
        try {
            ...
            logDebug("Get response code from bundle");
            ...
            if(intention == null) {
                logError("Unable to buy item, Intention listener is null, Error response: " + getResponseDesc(response));
                flagEndAsync();
                result = new IabResult(response, "Unable to buy item, intention listener is null");
                if (listener != null) listener.onIabPurchaseFinished(result, null);
                return;
            }

            logDebug("Get In-App purchase data from bundle");

            InAppPurchaseData inAppPurchaseData = getInAppPurchaseDataFromBundle(buyIntentBundle);
            if (inAppPurchaseData == null) {
                logError("Something went wrong with the buy intent data. Error response: " + getResponseDesc(response));
                flagEndAsync();
                result = new IabResult(response, "Unable to buy item, InAppPurchase data is null");
                if (listener != null) listener.onIabPurchaseFinished(result, null);
                return;
            }

            logDebug("Constructing pending buy intent");

            PendingIntent pendingIntent = buyIntentBundle.getParcelable(RESPONSE_BUY_INTENT);
            intention.onPendingIntentPrepared(inAppPurchaseData, pendingIntent);
...
//        TODO: Review if this code is really necessary
...
//        }
    ...
    InAppPurchaseData getInAppPurchaseDataFromBundle(Bundle b) throws JSONException {
        Object o = b.get(RESPONSE_INAPP_PURCHASE_DATA);
        if (o == null) {
            logError("Bundle with null in app purchase data");
            throw new RuntimeException("Unexpected type for bundle purchase data: " + o.getClass().getName());
        } else if (o instanceof String) {
            String purchaseDataStr = o.toString();

            JSONObject purchaseDataJson = new JSONObject(purchaseDataStr);
            Boolean autoRenewing = purchaseDataJson.getBoolean("translatedText");
            String orderId = purchaseDataJson.getString("orderId");
            String packageName = purchaseDataJson.getString("packageName");
            String productId = purchaseDataJson.getString("productId");
            String purchaseToken = purchaseDataJson.getString("purchaseToken");

            return new InAppPurchaseData(autoRenewing, orderId, packageName, productId, purchaseToken);
        } else {
            logError("Unexpected type for bundle purchase data.");
            logError(o.getClass().getName());
            throw new RuntimeException("Unexpected type for bundle purchase data: " + o.getClass().getName());
        }
    }
